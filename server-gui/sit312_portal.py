...
class Ui_MainWindow(object):
    ...
    def update_message_tab(self):
        for i in range(0, self.message_db_rows):
            ...
            item = QtWidgets.QTableWidgetItem()
            item.setText(QtWidgets.QApplication.translate("MainWindow", db_item.get('datetime'), None, -1))
            self.tableWidget.setItem(i, 1, item)   
    ...
    def update_audio_tab():

        # To do
        
        return 

    def insert_message(self, MainWindow, device_id, datetime, message):
        # Insert message (id: datetime: message: )
        item = QtWidgets.QTableWidgetItem()
        item.setText(QtWidgets.QApplication.translate("MainWindow", device_id, None, -1))
        self.tableWidget.setItem(self.message_db_rows - 1, 0, item)
    ...
    def broadcast_message(self, window):
        message = self.plainTextEdit.toPlainText()
        index = helper.insert_message(self.message_db, 'BROADCAST', datetime.datetime.now().strftime("%I:%M%p %B %d, %Y"), message)
        self.message_db_rows = index + 1    
        self.tableWidget.setRowCount(self.message_db_rows)
        self.insert_message(self, 'BROADCAST', datetime.datetime.now().strftime("%I:%M%p %B %d, %Y"), message)

    def direct_message(self, window):
        message = self.plainTextEdit_2.toPlainText()
        device_id = self.comboBox.currentText()
        index = helper.insert_message(self.message_db, device_id, datetime.datetime.now().strftime("%I:%M%p %B %d, %Y"), message)
        self.message_db_rows = index + 1    
        self.tableWidget.setRowCount(self.message_db_rows)
        self.insert_message(self, device_id, datetime.datetime.now().strftime("%I:%M%p %B %d, %Y"), message)
    ...
    def setupUi(self, MainWindow):
        ...
        # Recorder tab -> recorder groupbox -> destination label
        self.label_10 = QtWidgets.QLabel(self.groupBox_2)
        self.label_10.setGeometry(QtCore.QRect(10, 170, 131, 21))
        font = QtGui.QFont()
        font.setPointSize(11)
        self.label_10.setFont(font)
        self.label_10.setObjectName("label_10")

        self.label_10.setText(QtWidgets.QApplication.translate("MainWindow", "Destination ID:", None, -1))

        # Recorder tab -> recorder groupbox -> destination id dropdown
        self.comboBox_3 = QtWidgets.QComboBox(self.groupBox_2)
        self.comboBox_3.setGeometry(QtCore.QRect(140, 165, 121, 31))
        font = QtGui.QFont()
        font.setPointSize(11)
        self.comboBox_3.setFont(font)
        self.comboBox_3.setObjectName("comboBox_3")
        ...
        self.comboBox_3.setItemText(10, QtWidgets.QApplication.translate("MainWindow", "194873281A6", None, -1))
        self.comboBox_3.setItemText(11, QtWidgets.QApplication.translate("MainWindow", "497274130AB", None, -1))

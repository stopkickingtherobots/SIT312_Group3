...
def gps_locate(ser):
    ...
    while(True):
        ...
        x = ser.readline()     # Psuedo-code
        #print(x)
        x = x.decode()          # Psuedo-code
        x_arr = x.split(',')    # Psuedo-code
        #print(x_arr)
        if 'GNGGA' in x:
            print(x_arr)
            return x_arr[2],x_arr[4] #GNGGA Lat, Long
            
        # Should include some timout assuming 1 second hot start.
        # Also need to consider ~30 second cold start (new method?)
...
def xbee_slave():
    ...
    remote_device = RemoteXBeeDevice(device, XBee64BitAddress.from_hex_string('0013A2004155E2AB'))
    device.open()
